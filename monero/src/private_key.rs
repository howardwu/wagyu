...
mod tests {
    ...
    mod subaddress_mainnet {
        ...
        #[test]
        fn from_seed() {
            KEYPAIRS.iter().for_each(|(seed, (private_spend_key, private_view_key), _, major, minor, address)| {
                test_from_seed::<N>(
                    private_spend_key,
                    private_view_key,
                    address,
        ...
        #[test]
        fn from_private_spend_key() {
            KEYPAIRS.iter().for_each(|(_, (private_spend_key, private_view_key), _, major, minor, address)| {
                test_from_private_spend_key::<N>(
                    private_view_key,
                    address,
                    &Format::Subaddress(*major, *minor),
        ...
        #[test]
        fn to_str() {
            KEYPAIRS.iter().for_each(|(seed, (private_spend_key, private_view_key), _, major, minor, _)| {
                let private_key = MoneroPrivateKey::<N>::from_seed(seed, &Format::Subaddress(*major, *minor)).unwrap();
                test_to_str(private_spend_key, private_view_key, &private_key);
            });
        }
    }
