...
impl MoneroFormat {
    ...
    pub fn from_address(address: &[u8]) -> Result<Self, FormatError> {
        match address[0] {
            18 | 24 | 53 => Ok(Self::Standard),
            19 | 25 | 54 => {
                let mut data = [0u8; 8];
                data.copy_from_slice(&address[65..73]);
                Ok(Self::Integrated(data))
            }
            42 | 36 | 63 => Ok(Self::Subaddress(u32::max_value(), u32::max_value())),
            _ => return Err(FormatError::InvalidPrefix(vec![address[0]])),
        }
    }
}
...
impl fmt::Display for MoneroFormat {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::Integrated(payment_id) => write!(f, "integrated({})", hex::encode(payment_id)),
            Self::Subaddress(major, minor) => write!(f, "subaddress({},{})", major, minor),
        }
    }
}