...
impl<N: TronNetwork> Address for TronAddress<N> {
    ...
    /// Returns the address corresponding to the given Tron private key.
    fn from_private_key(private_key: &Self::PrivateKey, format: &Self::Format) -> Result<Self, AddressError> {
        ...
        match format {
            ...
            TronFormat::Bech32 => Self::bech32(&public_key),
        }
    }
...
impl<N: TronNetwork> TronAddress<N> {
    ...
    pub fn p2pkh(public_key: &<Self as Address>::PublicKey) -> Result<Self, AddressError> {
        ...
        let mut address = [0u8; 25];
        address[0] = N::to_address_prefix(&TronFormat::P2PKH)[0];
    ...
    pub fn p2wsh(_original_script: &Vec<u8>) -> Result<Self, AddressError> {
        ...
        Ok(Self {
            ...
            format: TronFormat::P2WSH,
            _network: PhantomData,
        })
    }

    /// Returns a P2SH_P2WPKH address from a given Tron public key.
    pub fn p2sh_p2wpkh(public_key: &<Self as Address>::PublicKey) -> Result<Self, AddressError> {
        let mut address = [0u8; 25];
        address[0] = N::to_address_prefix(&TronFormat::P2SH_P2WPKH)[0];
        ...
        Ok(Self {
            address: address.to_base58(),
            format: TronFormat::P2SH_P2WPKH,
            _network: PhantomData,
        })
    }

    /// Returns a Bech32 address from a given Tron public key.
    pub fn bech32(public_key: &<Self as Address>::PublicKey) -> Result<Self, AddressError> {
        ...
        Ok(Self {
            address: bech32.to_string(),
            format: TronFormat::Bech32,
            _network: PhantomData,
        })
    }

    /// Returns the format of the Tron address.
    pub fn format(&self) -> TronFormat {
        self.format.clone()
    }

    /// Returns a redeem script for a given Tron public key.
...
impl<N: TronNetwork> FromStr for TronAddress<N> {
    ...
    fn from_str(address: &str) -> Result<Self, Self::Err> {
        ...
        if let Ok(format) = TronFormat::from_address_prefix(prefix.as_bytes()) {
            if TronFormat::Bech32 == format {
        ...
        let format = TronFormat::from_address_prefix(&data[0..2])?;
...
mod tests {
    ...
    mod p2pkh_mainnet_compressed {
        ...
        fn from_private_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                ...
                test_from_private_key(address, &private_key, &TronFormat::P2PKH);
            });
        }
        ...
        fn from_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                ...
                test_from_public_key(address, &public_key, &TronFormat::P2PKH);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, address)| {
                test_from_str::<N>(address, &TronFormat::P2PKH);
            });
        }
    ...
    mod p2pkh_mainnet_uncompressed {
        ...
        fn test_invalid() {
            ...
            let private_key = TronPrivateKey::<N>::from_str(private_key).unwrap();
            let address = TronAddress::<N>::from_private_key(&private_key, &TronFormat::P2PKH).unwrap();
            assert_ne!(expected_address, address.to_string());

            let public_key = TronPublicKey::<N>::from_private_key(&private_key);
            let address = TronAddress::<N>::from_public_key(&public_key, &TronFormat::P2PKH).unwrap();
            assert_ne!(expected_address, address.to_string());
    ...
    mod p2sh_p2wpkh_mainnet {
        ...
        fn from_private_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = TronPrivateKey::<N>::from_str(private_key).unwrap();
                test_from_private_key(address, &private_key, &TronFormat::P2SH_P2WPKH);
            });
        }
        ...
        fn from_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = TronPrivateKey::<N>::from_str(private_key).unwrap();
                let public_key = TronPublicKey::<N>::from_private_key(&private_key);
                test_from_public_key(address, &public_key, &TronFormat::P2SH_P2WPKH);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, address)| {
                test_from_str::<N>(address, &TronFormat::P2SH_P2WPKH);
            });
        }
        ...
        fn test_invalid() {
            ...
            let private_key = TronPrivateKey::<N>::from_str(private_key).unwrap();
            let address = TronAddress::<N>::from_private_key(&private_key, &TronFormat::P2SH_P2WPKH).unwrap();
            assert_ne!(expected_address, address.to_string());

            let public_key = TronPublicKey::<N>::from_private_key(&private_key);
            let address = TronAddress::<N>::from_public_key(&public_key, &TronFormat::P2SH_P2WPKH).unwrap();
            assert_ne!(expected_address, address.to_string());
    ...
    mod bech32_mainnet {
        ...
        fn from_invalid_address() {
            INVALID.iter().for_each(|invalid_bech32| {
                assert_eq!(true, TronAddress::<N>::from_str(invalid_bech32).is_err());
            });
        }
        ...
        fn from_private_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = TronPrivateKey::<N>::from_str(private_key).unwrap();
                test_from_private_key(address, &private_key, &TronFormat::Bech32);
            });
        }
        ...
        fn from_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = TronPrivateKey::<N>::from_str(private_key).unwrap();
                let public_key = TronPublicKey::<N>::from_private_key(&private_key);
                test_from_public_key(address, &public_key, &TronFormat::Bech32);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, address)| {
                test_from_str::<N>(address, &TronFormat::Bech32);
            });
        }
    ...
    mod p2wsh_testnet {
        ...
        fn from_str() {
            SCRIPTPAIRS.iter().for_each(|(script, address)| {
                ...
                let new_address = TronAddress::<N>::p2wsh(&script_hex).unwrap();
                assert_eq!(new_address.to_string(), address.to_string());
                assert_eq!(new_address.format, TronFormat::P2WSH);
            });
        }
