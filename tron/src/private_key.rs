...
impl<N: TronNetwork> PrivateKey for TronPrivateKey<N> {
    ...
    /// Returns a randomly-generated compressed Tron private key.
...
impl<N: TronNetwork> FromStr for TronPrivateKey<N> {
    ...
    /// Returns a Tron private key from a given WIF.
...
mod tests {
    ...
    fn test_from_secp256k1_secret_key<N: TronNetwork>(
        expected_wif: &str,
        ...
        let private_key = TronPrivateKey::<N>::from_secp256k1_secret_key(&secret_key, compressed);
    ...
    fn test_from_str<N: TronNetwork>(
        ...
        let private_key = TronPrivateKey::<N>::from_str(wif).unwrap();
    ...
    mod p2pkh_mainnet_compressed {
        ...
        fn to_address() {
            KEYPAIRS.iter().for_each(|(private_key, _, address)| {
                ...
                let private_key = TronPrivateKey::<N>::from_str(&private_key).unwrap();
                test_to_address(&address, &TronFormat::P2PKH, &private_key);
            });
        }
    ...
    mod p2sh_p2wpkh_mainnet {
        ...
        fn to_address() {
            KEYPAIRS.iter().for_each(|(private_key, _, address)| {
                let address = TronAddress::<N>::from_str(address).unwrap();
                let private_key = TronPrivateKey::<N>::from_str(&private_key).unwrap();
                test_to_address(&address, &TronFormat::P2SH_P2WPKH, &private_key);
            });
        }
