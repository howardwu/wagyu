...
/// Represents a Tron signature hash
...
/// Represents a Tron transaction outpoint
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Outpoint<N: TronNetwork> {
    /// The previous transaction hash (32 bytes) (uses reversed hash order from Tron RPC)
    ...
    pub address: Option<TronAddress<N>>,
}

impl<N: TronNetwork> Outpoint<N> {
    /// Returns a new Tron transaction outpoint
    pub fn new(
        reverse_transaction_id: Vec<u8>,
        index: u32,
        address: Option<TronAddress<N>>,
        ...
        let (script_pub_key, redeem_script) = match address.clone() {
            Some(address) => {
                ...
                let redeem_script = match address.format() {
                    ...
                    TronFormat::P2WSH => match redeem_script {
                    ...
                    TronFormat::P2SH_P2WPKH => match redeem_script {
                    ...
                    TronFormat::Bech32 => match redeem_script.is_some() {
...
/// Represents a Tron transaction input
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct TronTransactionInput<N: TronNetwork> {
...
impl<N: TronNetwork> TronTransactionInput<N> {
    const DEFAULT_SEQUENCE: [u8; 4] = [0xff, 0xff, 0xff, 0xff];

    /// Returns a new Tron transaction input without the script (unlocking).
    pub fn new(
        ...
        Ok(Self {
            outpoint,
            script_sig: vec![],
            sequence: sequence.unwrap_or(TronTransactionInput::<N>::DEFAULT_SEQUENCE.to_vec()),
    ...
    /// Read and output a Tron transaction input
    pub fn read<R: Read>(mut reader: &mut R) -> Result<Self, TransactionError> {
        ...
        let script_sig: Vec<u8> = TronVector::read(&mut reader, |s| {
            let mut byte = [0u8; 1];
            s.read(&mut byte)?;
            Ok(byte[0])
        })?;
            ...
            false => match self.script_sig.len() {
                0 => match &self.outpoint.address {
                    Some(address) => match address.format() {
                        TronFormat::Bech32 => input.extend(vec![0x00]),
                        TronFormat::P2WSH => input.extend(vec![0x00]),
                        _ => {
...
/// Represents a Tron transaction output
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct TronTransactionOutput {
...
impl TronTransactionOutput {
    /// Returns a Tron transaction output.
    pub fn new<N: TronNetwork>(
        address: &TronAddress<N>,
        amount: TronAmount,
    ) -> Result<Self, TransactionError> {
        Ok(Self {
            amount,
    ...
    /// Read and output a Tron transaction output
    pub fn read<R: Read>(mut reader: &mut R) -> Result<Self, TransactionError> {
        let mut amount = [0u8; 8];
        reader.read(&mut amount)?;

        let script_pub_key: Vec<u8> = TronVector::read(&mut reader, |s| {
            let mut byte = [0u8; 1];
            s.read(&mut byte)?;
            Ok(byte[0])
        })?;
        ...
        Ok(Self {
            amount: TronAmount::from_satoshi(u64::from_le_bytes(amount) as i64)?,
            script_pub_key,
        })
    }
...
/// Represents an Tron transaction id and witness transaction id
...
/// Represents the Tron transaction parameters
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct TronTransactionParameters<N: TronNetwork> {
    /// The version number (4 bytes)
    pub version: u32,
    /// The transaction inputs
    pub inputs: Vec<TronTransactionInput<N>>,
    /// The transaction outputs
    pub outputs: Vec<TronTransactionOutput>,
...
impl<N: TronNetwork> TronTransactionParameters<N> {
    /// Read and output the Tron transaction parameters
    pub fn read<R: Read>(mut reader: R) -> Result<Self, TransactionError> {
        let mut version = [0u8; 4];
        reader.read(&mut version)?;

        let mut inputs = TronVector::read(&mut reader, TronTransactionInput::<N>::read)?;
        ...
        let segwit_flag = match inputs.is_empty() {
            true => {
                ...
                match flag[0] {
                    1 => {
                        inputs = TronVector::read(&mut reader, TronTransactionInput::<N>::read)?;
                        true
                    }
        ...
        let outputs = TronVector::read(&mut reader, TronTransactionOutput::read)?;

        if segwit_flag {
            for input in &mut inputs {
                let witnesses: Vec<Vec<u8>> = TronVector::read(&mut reader, |s| {
                    let (size, witness) = TronVector::read_witness(s, |sr| {
                        let mut byte = [0u8; 1];
                        sr.read(&mut byte)?;
                        Ok(byte[0])
                    })?;
        ...
        for (vin, input) in self.parameters.inputs.iter().enumerate() {
            ...
            let address_is_valid = match &address.format() {
                TronFormat::P2WSH => {
                    ...
                    let c_address = TronAddress::<N>::p2wsh(&input_script)?;
                    address == &c_address
                },
            ...
            if address_is_valid && !transaction.parameters.inputs[vin].is_signed {
                ...
                let preimage = match &address.format() {
                    TronFormat::P2PKH => transaction.p2pkh_hash_preimage(vin, input.sighash_code)?,
...
impl<N: TronNetwork> TronTransaction<N> {
    ...
    pub fn segwit_hash_preimage(&self, vin: usize, sighash: SignatureHash) -> Result<Vec<u8>, TransactionError> {
        ...
        let script = match format {
            ...
            TronFormat::P2WSH => match &input.outpoint.redeem_script {
                Some(redeem_script) => redeem_script.to_vec(),
                None => return Err(TransactionError::InvalidInputs("P2WSH".into())),
            },
            TronFormat::P2SH_P2WPKH => match &input.outpoint.redeem_script {
                Some(redeem_script) => redeem_script[1..].to_vec(),
                None => return Err(TransactionError::InvalidInputs("P2SH_P2WPKH".into())),
            },
            TronFormat::P2PKH => return Err(TransactionError::UnsupportedPreimage("P2PKH".into())),
        };

        let mut script_code = vec![];
        if format == TronFormat::P2WSH {
...
mod tests {
    ...
    fn test_multisig_transaction<N: TronNetwork>(
        version: u32,
        lock_time: u32,
        inputs: Vec<Input>,
        outputs: Vec<Output>,
        ...
        for input in &inputs {
            let private_key = TronPrivateKey::from_str(input.private_key).unwrap();
            ...
            let redeem_script = match (input.redeem_script, input.address_format.clone()) {
                (Some(script), TronFormat::P2WSH) => Some(hex::decode(script).unwrap()),
            ...
            let address = match &input.address_format {
                TronFormat::P2WSH => TronAddress::<N>::p2wsh(redeem_script.as_ref().unwrap()).unwrap(),
            ...
            let mut transaction_input = TronTransactionInput::<N>::new(
                transaction_id,
                input.index,
                Some(address),
        ...
        for output in outputs {
            let address = TronAddress::<N>::from_str(output.address);
            if address.is_ok() {
                output_vec.push(TronTransactionOutput::new(&address.unwrap(), output.amount).unwrap());
            } else {
                let tx_output = TronTransactionOutput {
        ...
        let mut transaction = TronTransaction::<N>::new(&transaction_parameters).unwrap();

        // Sign transaction
        for input in inputs {
            transaction = transaction
                .sign(&TronPrivateKey::from_str(input.private_key).unwrap())
                .unwrap();
        }
    ...
    fn test_transaction<N: TronNetwork>(
        version: u32,
        lock_time: u32,
        inputs: Vec<Input>,
        outputs: Vec<Output>,
        ...
        let mut output_vec = vec![];
        for output in outputs {
            let address = TronAddress::<N>::from_str(output.address).unwrap();
            output_vec.push(TronTransactionOutput::new(&address, output.amount).unwrap());
        }
    ...
    fn test_reconstructed_transaction<N: TronNetwork>(
        version: u32,
        lock_time: u32,
        inputs: Vec<Input>,
        outputs: Vec<Output>,
        ...
        let transaction = TronTransaction::<N>::new(&transaction_parameters).unwrap();
        let unsigned_raw_transaction = hex::encode(&transaction.to_transaction_bytes().unwrap());

        let mut new_transaction = TronTransaction::<N>::from_str(&unsigned_raw_transaction).unwrap();
        ...
        for input in inputs {
            ...
            new_transaction = TronTransaction::<N>::from_str(&partial_signed_transaction).unwrap();
            ...
            TransactionTestCase { // p2sh_p2wpkh to p2pkh - based on https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#p2sh-p2wpkh
                ...
                outputs: &[
                    Output {
                        ...
                        amount: TronAmount(199996600)
                    },
                    Output {
                        address: "1Q5YjKVj5yQWHBBsyEBamkfph3cA6G9KK8",
                        amount: TronAmount(800000000)
                    },
                ],
            ...
            TransactionTestCase { //p2sh_p2wpkh to segwit
                ...
                outputs: &[
                    Output {
                        ...
                        amount: TronAmount(100000000)
                    },
                    Output {
                        address: "3MSu6Ak7L6RY5HdghczUcXzGaVVCusAeYj",
                        amount: TronAmount(899990000),
                    },
                ],
            ...
            TransactionTestCase { // p2pkh and p2sh_p2wpkh to p2pkh - based on https://github.com/bitcoinjs/bitcoinjs-lib/blob/master/test/integration/transactions.js
                ...
                outputs: &[
                    Output {
                        ...
                        amount: TronAmount(180000)
                    },
                    Output {
                        address: "1JtK9CQw1syfWj1WtFMWomrYdV3W2tWBF9",
                        amount: TronAmount(170000)
                    },
                ],
            ...
            TransactionTestCase { // p2sh_p2wsh to multiple address types
                ...
                inputs: &[
                    Input {
                        ...
                        redeem_script: None,
                        script_pub_key: None,
                        utxo_amount: TronAmount(2000000),
                        sequence: None,
                        sighash_code: SignatureHash::SIGHASH_ALL
                    },
                ],
                outputs: &[
                    Output {
                        ...
                        amount: TronAmount(30000)
                    },
                    Output {
                        address: "1NxCpkhj6n8orGdhPpxCD3B52WvoR4CS7S",
                        amount: TronAmount(2000000)
                    },
                ],
            ...
            TransactionTestCase { // p2sh_p2wsh and p2pkh to multiple address types
                ...
                outputs: &[
                    Output {
                        ...
                        amount: TronAmount(50000)
                    },
                    Output {
                        address: "17rB37JzbUVmFuKMx8fexrHjdWBtDurpuL",
                        amount: TronAmount(123456)
                    },
                ],
            ...
            TransactionTestCase { // p2sh-p2wpkh to bech32
                ...
                inputs: &[
                    Input {
                        ...
                        redeem_script: None,
                        script_pub_key: None,
                        utxo_amount: TronAmount(1500000),
                        sequence: None,
                        sighash_code: SignatureHash::SIGHASH_ALL
                    },
                ],
    ...
    mod test_invalid_transactions {
        ...
        fn test_invalid_inputs() {
            for input in INVALID_INPUTS.iter() {
                ...
                let private_key = TronPrivateKey::<N>::from_str(input.private_key);
                match private_key {
                    Ok(private_key) => {
                        let address = private_key.to_address(&input.address_format).unwrap();
                        let invalid_input = TronTransactionInput::<N>::new(
                            transaction_id,
                            input.index,
                            Some(address),
                            Some(input.utxo_amount),
            ...
            TransactionTestCase { // Transaction 2 -> P2WSH to Bech32(P2WPKH) and itself
                ...
                inputs: &[
                    ...
                    Input {
                        ...
                        address_format: TronFormat::P2WSH,
                        transaction_id: "76ef90fa70e4c10adc358432a979683a2cf1855ff545f88c5022dea8863ed5ab",
                        index: 0,
