...
impl TronNetwork for Testnet {
    ...
    fn to_address_prefix(format: &TronFormat) -> Vec<u8> {
        match format {
            ...
            TronFormat::Bech32 => vec![0x74, 0x62],
        }
    }
    ...
    fn to_extended_private_key_version_bytes(format: &TronFormat) -> Result<Vec<u8>, ExtendedPrivateKeyError> {
        match format {
            TronFormat::P2PKH => Ok(vec![0x04, 0x35, 0x83, 0x94]), // tpriv
            TronFormat::P2SH_P2WPKH => Ok(vec![0x04, 0x4A, 0x4E, 0x28]), // upriv
    ...
    fn to_extended_public_key_version_bytes(format: &TronFormat) -> Result<Vec<u8>, ExtendedPublicKeyError> {
        match format {
            TronFormat::P2PKH => Ok(vec![0x04, 0x35, 0x87, 0xCF]), // tpub
            TronFormat::P2SH_P2WPKH => Ok(vec![0x04, 0x4A, 0x52, 0x62]), // upub
