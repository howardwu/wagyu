...
fn ethereum_address_from_private_key() {
    ...
    let address = EthereumAddress::from_private_key(&private_key, None);
...
fn ethereum_address_from_public_key() {
    ...
    let address = EthereumAddress::from_public_key(&public_key, None);
...
fn ethereum_private_key_to_address() {
    let private_key = ethereum_get_private_key();
    let address = private_key.to_address(None);
...
fn ethereum_public_key_to_address() {
    ...
    assert_eq!(public_key.to_address(None).to_string(), ADDRESS_STR);
}
...
fn ethereum_get_private_key() -> EthereumPrivateKey {
    ...
    let secret_key = secp256k1::SecretKey::from_slice(&secp256k1::Secp256k1::new(), &secret_key_bytes).unwrap();

    EthereumPrivateKey { secret_key, wif: PRIVATE_KEY_STR.to_string() }
}

fn ethereum_get_public_key() -> EthereumPublicKey {
    let public_key_bytes = hex::decode(PUBLIC_KEY_STR).unwrap();
    let public_key = secp256k1::PublicKey::from_slice(&secp256k1::Secp256k1::new(), &public_key_bytes).unwrap();
