...
mod tests {
    ...
    fn test_to_string(expected_extended_public_key: &str) {
        let extended_public_key = EthereumExtendedPublicKey::from_str(&expected_extended_public_key).unwrap();
        assert_eq!(expected_extended_public_key, extended_public_key.to_string());
    }

    fn test_invalid_secret_key(extended_public_key: &str) {
        let secret_key =
    ...
    fn test_invalid_length(extended_public_key: &str) {
        ...
        assert!(EthereumExtendedPublicKey::from_str(short).is_err());
        assert!(EthereumExtendedPublicKey::from_str(&long).is_err());
    }
    ...
    mod bip32_mainnet {
        ...
        fn from_extended_private_key() {
            KEYPAIRS.iter().for_each(
                |(
                    ...
                    test_from_extended_private_key(
                        extended_public_key,
                        public_key,
        ...
        fn to_string() {
            KEYPAIRS.iter().for_each(|(_, _, _, _, _, _, _, extended_public_key)| {
                test_to_string(extended_public_key);
            });
        }
        ...
        fn invalid_secret_key() {
            KEYPAIRS.iter().for_each(|(_, _, _, _, _, _, _, extended_public_key)| {
                test_invalid_secret_key(extended_public_key);
            })
        }
        ...
        fn invalid_version() {
            KEYPAIRS.iter().for_each(|(_, _, _, _, _, _, _, extended_public_key)| {
                test_invalid_version(extended_public_key);
            })
        }
        ...
        fn invalid_checksum() {
            KEYPAIRS.iter().for_each(|(_, _, _, _, _, _, _, extended_public_key)| {
                test_invalid_checksum(extended_public_key);
            })
        }
        ...
        fn invalid_length() {
            KEYPAIRS.iter().for_each(|(_, _, _, _, _, _, _, extended_public_key)| {
                test_invalid_length(extended_public_key);
            })
        }
    }
}
