...
pub fn prime_field(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let repr_ident =
        fetch_wrapped_ident(&ast.data).expect("PrimeField derive only operates over tuple structs of a single item");
...
fn test_exp() {
    assert_eq!(
        exp(
            ...
            &BigUint::from_str("52435875175126190479447740508185965837690552500527637822603658699938581184513")
                .unwrap()
        ),
