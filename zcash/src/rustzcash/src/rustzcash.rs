...
pub fn get_point(
    bytes: *const [c_uchar; 32],
) -> Result<edwards::Point<Bls12, PrimeOrder>, ()> {


    let point = match edwards::Point::<Bls12, Unknown>::read(&(unsafe { &*bytes })[..], &JUBJUB) {
        Ok(p) => p,
        Err(_) => return Err(()),
    };

    let point = match point.as_prime_order(&JUBJUB) {
        Some(point) => point,
        None => return Err(()),
    };
