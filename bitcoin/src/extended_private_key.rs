...
use sha2::Sha512;
use std::{fmt, fmt::Display, io::Cursor, marker::PhantomData, str::FromStr};
...
impl<N: BitcoinNetwork> ExtendedPrivateKey for BitcoinExtendedPrivateKey<N> {
    ...
    fn new(seed: &[u8], format: &Self::Format) -> Result<Self, ExtendedPrivateKeyError> {
        ...
        Ok(Self::new_master(seed)?.derive(&path)?)
    }
...
mod tests {
    ...
    fn test_new<N: BitcoinNetwork>(
        ...
        let extended_private_key =
            BitcoinExtendedPrivateKey::<N>::new(&hex::decode(seed).unwrap(), format).unwrap();
            ...
            KEYPAIRS.iter().for_each(
                |(path, seed, child_index, secret_key, chain_code, parent_fingerprint, extended_private_key, _)| {
                    test_new::<N>(
                        ...
                        secret_key,
                        seed,
                        &BitcoinFormat::from_path(path, BitcoinFormat::P2PKH).unwrap(),
                    );
                },
            );
        }
            ...
            KEYPAIRS.iter().for_each(
                |(path, seed, child_index, secret_key, chain_code, parent_fingerprint, extended_private_key, _)| {
                    test_new::<N>(
                        ...
                        secret_key,
                        seed,
                        &BitcoinFormat::from_path(path, BitcoinFormat::P2SH_P2WPKH).unwrap(),
                    );
                },
            );
        }
